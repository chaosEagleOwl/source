...
AsdkSquare::subList() const
{
    ...
    // Must call subList() rather than list() because if this method
    // was called from an execution of list(), then we'll end up in an
    // infinite recursion which will cause a crash when the stack
    // overflows.
    //
	AcDbEntity::subList();
